	...
	<tbody>
		<tr>
			<td>
				...
				<th align="left">Cargar Datos:</th> 
				<td bgcolor="#ffffff">
					<input type="file" id="cargar_doc" placeholder="Cargar Documento de Precios Clientes">
				</td>
				<td bgcolor="#ffffff">
					<button type="button" class="btn btn-primary" id="cargar_save">Cargar</button>
				</td>
			</td>
		</tr>
	</tbody>
...
<script>
		...
		reader.onload = function(e) {
			...
			var nuevoJSON = jsonData.map(function(item) {
				return {
					...
					materiales: item.Codigo,
					montoMaterial: item.
					precioUnitario: ite.PRECIO A CARGAR
				};
			});
			callback(jsonData);
		};
	...
	$('#cargar_save').on('click', function() {
		var file = $('#cargar_doc')[0].files[0];

		obtenerJSONDesdeExcel(file, function(jsonData) {
			datos = jsonData;
			console.log(jsonData)
			// Aquí puedes realizar cualquier acción con el nuevo JSON obtenido
		});
	});
